...
public class HandlerRoomServerReserve{
    public static void handleReserveRoom(PrintWriter out, String body, String path) throws IOException, SQLException {
        ...
        if(isAvailable){ //that if statement executes when the room doesn't exist before so it is added to database
            response = "<HTML>\n" +
                    ...
                    "</HEAD>\n" +
                    "<BODY> Room with name " + parameters.get("name") + " is successfully reserved at " +  wantedDay + " between " + wantedHour + ":00 - " + wantedFinishHour
                    + ":00 Your reservation id is "
                    + handleGetReservationId(parameters.get("name"),parameters.get("day"),parameters.get("hour"), parameters.get("duration"), parameters.get("activity")) + "</BODY>\n" +
                    "</HTML>";
    ...
    public static boolean handleInsert(Object name,Object day,Object hour,Object duration, Object activity, boolean isAvailable) throws SQLException {
        ...
        ResultSet rs = stmt.executeQuery( "SELECT * FROM public.reservation;" ); // fetch the existing reservations
        int i=0;
        while ( rs.next() ) {
    ...
    public static int handleGetReservationId(Object name,Object day,Object hour,Object duration, Object activity) throws SQLException {
        connectDatabase(); //opened the postgresql connection
        Connection c = DriverManager
                .getConnection("jdbc:postgresql://localhost:5432/postgres",
                        "alper", "pass"); //get the connection
        ...
        int wantedDay = Integer.parseInt(day.toString()); //the day that wanted to be reserved
        int wantedHour = Integer.parseInt(hour.toString()); //the hour that wanted to be reserved
        ResultSet rs = stmt.executeQuery( "SELECT id FROM public.reservation where \"room\" = '"  + name.toString() + "' and \"day\" = " + wantedDay + " and \"hour\" = " + wantedHour + ";" ); // fetch the reservation id
        int resId = 0;
        if(rs.next()) resId = rs.getInt("id");
        return resId;
    }
